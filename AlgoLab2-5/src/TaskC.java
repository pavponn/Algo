       ...
       // p = in.nextInt();
        ...
        for (int i = 1; i < n + 1; i++) {
            for (int j = 1; j < k + 1; j++) {
                ...
                matrix[i][j] = random.nextInt(1000000);
            }
        }
    ...
    int dfs(int v) {
        ...
        for (int prevColor = 1; prevColor < k + 1; prevColor++) {
            for (int color = 1; color < k + 1; color++) {
                ...
                if (!children.get(v).isEmpty() && children.get(v).size() < k || v == 1 && children.get(v).size() < k + 1) {
