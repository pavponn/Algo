...
int main() {
    ...
    in >> ind;
    buildArray(str);
    std::vector<int> p = prefixFunction(str);
    int k = str.size() - p[str.size() - 1];
    int period = 0;
    int c =  str.size() / k;
    if (str.size() % k != 0) {
        k = str.size();
    ...
    int suffixPos = mSuffArray[c * (ind - 1)];
    std::string result = str.substr(suffixPos, str.length() - suffixPos);
    result += (str.substr(0, suffixPos));
    out << result << std:: endl;
    return 0;
}